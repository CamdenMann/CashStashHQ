...
(function () {
  const CONFIG = {
    ...
    sponsorFormUrl: "#partner", // Replace later with your real website form link, like https://cashstash.com/#partner
    rulesUrl: "rules.html",
  ...
  const leadSteps = [
    ...
    { key: "phone", q: "Optional but helpful — what phone number should they use if texting/calling is easier? If you don’t want to share, type “skip.”" },
  ...
  function createWidget() {
    ...
    widget.innerHTML = `
      <div class="cs-chat-head">
        ...
        <button class="cs-close" type="button">✕</button>
      </div>
    ...
    widget.querySelector(".cs-close").addEventListener("click", () => widget.classList.remove("cs-open"));
  ...
  function body() { return document.getElementById("csChatBody"); }
  ...
  function respond(raw) {
    ...
    if (matches(text, ["sponsor all", "all cities", "network", "multi city", "multiple cities"])) {
      ...
      bot("For full-network interest, Cash Stash HQ will usually want to understand your goal, cities, timeline, and budget before recommending the right structure.");
    ...
    if (matches(text, ["calculator", "recommend", "best campaign", "which campaign", "campaign type"])) {
    ...
    if (matches(text, ["what does sponsor get", "benefits", "what do i get", "exposure"])) {
      bot("Sponsors can get shoutouts, tags, traffic to their page or site, winner content, city-specific visibility, and multi-city exposure. The value is social reach + real-world participation combined.");
    ...
    if (matches(text, ["results", "roi", "views", "turnout", "show up", "work"])) {
      ...
      bot("Safe proof points: the network has over 100K followers, some posts reach hundreds of thousands of views, drops can bring 100+ people, and strong campaigns can reach around 1M views across pages.");
    ...
    if (matches(text, ["cities", "locations", "where are you", "active cities"])) {
    ...
    if (cityKey) {
      ...
      htmlBot(`<a class="cs-quick" href="${CONFIG.cityPages[cityKey]}" target="_blank">Open ${titleCity(cityKey)} page</a> <a class="cs-quick" href="${CONFIG.cityInstagram[cityKey]}" target="_blank">Open Instagram</a>`);
    ...
    if (matches(text, ["rules", "safe", "trespass", "digging", "damage", "minor", "minors"])) {
      bot(`Basic rules: no trespassing, no digging, no damaging property, and don’t do anything unsafe. Full rules are here: ${CONFIG.rulesUrl}`);
      return;
    }
    ...
    if (matches(text, ["contact", "form", "get started", "submit", "partner"])) {
      ...
      htmlBot(`<a class="cs-quick" href="${CONFIG.sponsorFormUrl}" target="_blank">Open sponsor form</a>`);
      return;
    }
  ...
  function budgetGuidance(lead, rec) {
    ...
    if (!n) {
      return "Budget note: if you’re not sure on budget yet, Cash Stash HQ can recommend options based on city count and campaign size.";
    }
  ...
  function findCityKey(text) {
    const keys = Object.keys(CONFIG.cityPages);
    return keys.find(k => text.includes(k));
  }
